...
function buildPrompts(ocrText: string, hasInvoiceImage: boolean) {
  const system = [
    ...
    "",
    "CUSTOMER / ISSUER company name (critical — read from IMAGE layout, not from guessed table text):",
    "- customer.name = the company that ISSUED the invoice (seller), usually in the letterhead / address block ABOVE the line-items table.",
    "- German examples: GmbH, gGmbH, AG, KG, UG, e.V., e.K.",
    "- Turkish examples: A.Ş., Ltd. Şti., Ltd., Sti., San. Tic., Holding, …",
    "- NEVER use table column headers as customer.name: not 'Pos.', 'Menge', 'Bezeichnung', 'Einzelpreis', 'Gesamtpreis', 'Miktar', 'Açıklama', 'Tutar', etc.",
    "- If the OCR_TEXT block is empty or placeholder, IGNORE it for names — use ONLY the invoice image for customer.name and address.",
    "- If unsure, set customer.name to null (do not guess a table header).",
    "",
